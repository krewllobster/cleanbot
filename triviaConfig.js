...
const formatQuestion = (q, c) => {
  ...
  const {question, category, type, difficulty, correct_answer, incorrect_answers} = q
...
const populateQuestions = async () => {
  ...
  const {data: {token}} = await axios.get('https://opentdb.com/api_token.php?command=request')
  ...
  for (let i = 0; i < categories.length; i ++) {
    let c = categories[i]
    let {data: {category_question_count: {total_question_count: qs}}} = await axios.get(`https://opentdb.com/api_count.php?category=${c._id}`)
